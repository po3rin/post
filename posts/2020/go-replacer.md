...
こんにちはponです。今回は、Go の文字列置換周りのコードを読んだので知見を共有します。Goには正規表現を除くと2つの文字列置換が用意されています。```func (*Replacer) Replacer``` と ```func Replace``` です。今回はその2つの文字列置換のコードリーディングをしたのでその知見を共有します。
