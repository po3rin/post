...
こんにちは [pon](https://twitter.com/home) です。***Kubernetes*** が盛り上がるこの頃ですが、弊社の記事データパイプラインを ***EKS*** に移行するに辺り、発生したコード改修について紹介します。Kubernetes移行を検討するにあたって、KubernetesのManifestを淡々と書いていくことも大切ですが、場合によってはアプリケーションコードの改修も発生します。今回は破棄容易性の観点から、弊社のk8s移行で発生したコード修正を紹介します。
